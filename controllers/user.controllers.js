...
/* export const uploads = async (req, res) => {
  ...
  const filepath =
    "./uploads/black-white-painting-black-white-painting-with-black-background.jpg";
  const objectName = "img.jpg";
  const exists = await minioClient.bucketExists(bukectName).catch(() => false);
  if (!exists) {
    await minioClient.makeBucket(bukectName);
    console.log("doesn't exit");
  }
  minioClient.fPutObject(bukectName, objectName, filepath, (error, etag) => {
    if (error) {
      console.log(error);
    }
    res.send(etag);
...
//downoloading file from s3 bucket
